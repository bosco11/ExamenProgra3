...
public class DistritoController {
    ...
    public ResponseEntity<?> create(@Valid @RequestBody DistritoDTO DistritoDTO, BindingResult bindingResult) {
        if (!bindingResult.hasErrors()) {
            try {
                return new ResponseEntity(IDistritoService.create(DistritoDTO), HttpStatus.CREATED);
            } catch (Exception e) {
                return new ResponseEntity(e, HttpStatus.INTERNAL_SERVER_ERROR);
            }
        } else {
    ...
    public ResponseEntity<?> update(@PathVariable(value = "id") Long id, @Valid @RequestBody DistritoDTO DistritoDTO, BindingResult bindingResult) {
        if (!bindingResult.hasErrors()) {
            try {
                Optional<DistritoDTO> usuarioUpdated = IDistritoService.update(DistritoDTO, id);
    ...
    public ResponseEntity<?> findByEstadoContaining(@PathVariable(value = "term") boolean term) {
        try {
            return new ResponseEntity<>(IDistritoService.findByEstado(term), HttpStatus.OK);
        } catch (Exception e) {
            return new ResponseEntity<>(e, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
